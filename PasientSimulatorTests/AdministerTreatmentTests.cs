...
public class AdministerTreatmentTests
{
    ...
    {
        var allergicAdministeration = _administerTreatment.AdministerMedicine(wrongMedication, p1);

        Assert.That(allergicAdministeration, Is.False,
            "The wrong medication was administered, the patient was allergic and is now severely sick");

        Assert.That(p1.Status, Is.EqualTo(Patient.StatusEnum.SeverelySick));

        var wrongAdministration = _administerTreatment.AdministerMedicine(noEffectMedication, p1);

        Assert.That(wrongAdministration, Is.False, "The wrong medication was administered, it had no effect...");

        var correctAdministration = _administerTreatment.AdministerMedicine(influenzaMedication, p1);

        Assert.That(correctAdministration, Is.True,
            "The correct medicine was administered, the patient is now healthy!");
